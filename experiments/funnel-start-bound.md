...
7일 p50은 40.8→11.5ms, 30일 68.1→43.8ms, 전체 192.0→195.0ms였다.
...
2026-09-08 기록 점검: 이 API 스크립트의 percentile은 nearest-rank 방식
(`ceil(n × q)`번째 표본)이다. p50은 40개 중 20번째 값이며 가운데 두 값의
평균이 아니다. SQL p50도 20개 중 10번째 값이다. 저장 표본과 산출 코드로
확인했고, 위 수치를 유지하되 “중앙값” 표현을 p50으로 명확히 했다.

앞뒤 20회로 나눠도 짧은 기간의 개선 방향이 같았다. 변경하지 않은 Overview
API p50은 같은 순서로 25.7→25.4, 75.2→74.5, 174.6→177.8,
